		...
		switch(rxBuf.data[0]) {
			...
			case NILE_FLASH_CMD_READ:
				if(rxBuf.pos >= 4) {
					...
					if(mode == NILE_FLASH_CMD_WRITE) {
						printf("nileswan/spi/flash: write starting at location %06X\n", position);
					} else if(mode == NILE_FLASH_CMD_READ) {
						printf("nileswan/spi/flash: read starting at location %06X\n", position);
					} else {
						printf("nileswan/spi/flash: erasing %d bytes at location %06X\n", size, position);
					}
				}
				break;
