...
WsCartNileswan::WsCartNileswan(int _psram_banks, int _sram_banks) : WsCart(),
																						  psram_banks(_psram_banks),
																						  sram_banks(_sram_banks),
																						  flash(NileFlash()),
																						  mcu(NileMCU(this)),
																						  tf(NileTF(this))
{
...
{
	_state.SelectedBanks[0] = 0x00FF;
	_state.SelectedBanks[1] = 0xFFFF;
	_state.SelectedBanks[2] = 0xFFFF;
	_state.SelectedBanks[3] = 0xFFFF;
...
uint8_t WsCartNileswan::ReadPort(uint16_t index)
{
	...
	if(_nstate.PowCnt & NILE_POW_IO_2003) {
		switch(index) {
			case 0xCA:
			case 0xCB:
				return _cartRtc->ReadPort(index);
			case IO_CART_FLASH:
			...
			case IO_BANK_2003_RAM + 1:
				return _state.SelectedBanks[1] >> 8;
			case IO_BANK_2003_ROM0:
				return _state.SelectedBanks[2];
			case IO_BANK_2003_ROM0 + 1:
				return _state.SelectedBanks[2] >> 8;
			case IO_BANK_2003_ROM1:
				return _state.SelectedBanks[3];
			case IO_BANK_2003_ROM1 + 1:
				return _state.SelectedBanks[3] >> 8;
		}
	}
...
void WsCartNileswan::WritePort(uint16_t index, uint8_t value)
{
	...
	if(_nstate.PowCnt & NILE_POW_IO_2003) {
		switch(index) {
			case 0xCA:
			case 0xCB:
				_cartRtc->WritePort(index, value);
				break;
			case IO_CART_FLASH:
...
uint16_t WsCartNileswan::ResolveBankValue(int cpuBank) const
{
	...
	if(cpuBank == 1) {
		return _state.SelectedBanks[1] & ((_nstate.BankMask & NILE_SEG_SRAM_LOCK) ? maskOn : maskOff);
	} else if(cpuBank == 2) {
		return _state.SelectedBanks[2] & ((_nstate.BankMask & NILE_SEG_ROM0_LOCK) ? maskOn : maskOff);
	} else if(cpuBank == 3) {
		return _state.SelectedBanks[3] & ((_nstate.BankMask & NILE_SEG_ROM1_LOCK) ? maskOn : maskOff);
	} else {
